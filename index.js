...
module.exports = function (handle, options, cb) {
  ...
  function parseEnvelope (buf) {
    ...
    envelope.mflag = buf.slice(2, 4)
    var mflag = envelope.mflag.readUInt8()
    if ((mflag & ENV_FLAG_COMPRESSED) === ENV_FLAG_COMPRESSED) envelope.compressed = true
    if ((mflag & ENV_FLAG_ENCRYPTED) === ENV_FLAG_ENCRYPTED) envelope.encrypted = true
    if ((mflag & ENV_FLAG_TRUNCATED) === ENV_FLAG_TRUNCATED) envelope.truncated = true
  ...
  function parseBody (header, buf) {
    ...
    if (header.opFlag[MSG_FLAG_RRDG_INDEX] & MSG_FLAG_RRDG > 0) {
      ...
      if (digestLen) {
        offset = 1 + digestLen
        body.digest = buf.slice(1, offset)
      }
    }
  ...
  function parseHSSite (buf) {
    ...
    site.version = buf.slice(0, 2)
    site.protocolVersion = buf.slice(2, 4)
    site.serialNumber = buf.slice(4, 6)
    site.primaryMask = buf.slice(6, 7)
    site.hashOption = buf.slice(7, 8)
    ...
    for (var i = 0; i < serverCount; i++) {
      ...
      offset = offset + 4
      record.publicKey = buf.slice(offset, offset + pubkeyLength)
      offset = offset + pubkeyLength
      var interfaceCount = buf.slice(offset, offset + 4).readUInt32BE()
      offset = offset + 4
      for (var j = 0; j < interfaceCount; j++) {
        var iface = {}
        iface.serviceType = buf.slice(offset, offset + 1)
        iface.transmissionProtocol = buf.slice(offset + 1, offset + 2)
